...
const SortableHeader: React.FC<{
    ...
    return (
        <th 
            className={`px-6 py-3 text-[11px] font-extrabold text-gray-400 uppercase tracking-wider cursor-pointer hover:text-white hover:bg-white/5 transition-all normal-case font-sans ${className}`}
                    ...
                    <div className="overflow-x-auto rounded-xl shadow-xl border border-gray-700/50">
                        <table className="min-w-full divide-y divide-gray-800/50">
                            <thead className="bg-gray-950/30">
                                <tr>
                                    <th className="px-6 py-3 w-12 text-center sticky left-0 z-10 bg-inherit shadow-[4px_0_10px_rgba(0,0,0,0.2)]">
                                        <CircularCheckbox 
                            ...
                            <tbody className="divide-y divide-gray-800/30">
                                {filteredAndSortedGroups.length > 0 ? filteredAndSortedGroups.map((group) => {
                                    ...
                                    return (
                                        <tr 
                                            key={group.id} 
                                            className={`group transition-all duration-200 ${isSelected ? 'bg-indigo-900/20' : 'hover:bg-white/[0.03]'}`}
                                            onClick={() => handleToggleRow(group.id)}
                                        >
                                            <td className="px-6 py-2 align-middle whitespace-nowrap sticky left-0 z-10 bg-inherit">
                                                <CircularCheckbox 
                                                    checked={isSelected}
                                            ...
                                            {isVisible('createdAt') && (
                                                <td className="px-6 py-2 align-middle whitespace-nowrap">
                                                    <div className="flex flex-col">
                                                        ...
                                                        <span className="text-[9px] text-gray-500 opacity-60">{new Date(group.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                                                    </div>
                                                </td>
                                            )}
                                            {isVisible('channelCount') && (
                                                <td className="px-6 py-2 align-middle whitespace-nowrap text-center">
