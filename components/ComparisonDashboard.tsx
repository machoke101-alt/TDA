...
const StatBar: React.FC<{ value: number, max: number, label: string, gradientClass?: string }> = ({ value, max, label, gradientClass = "bg-indigo-600" }) => {
    const percentage = max > 0 ? (value / max) * 100 : 0;
    return (
        <div className="w-full bg-gray-800/60 rounded-full h-1.5 relative overflow-hidden border border-white/5">
            <div
                className={`${gradientClass} h-full rounded-full transition-all duration-500 ease-out shadow-[0_0_8px_rgba(0,0,0,0.3)]`}
                style={{ width: `${Math.max(percentage, 2)}%` }}
            ></div>
            <span className="absolute inset-0 flex items-center justify-start pl-2 text-[10px] font-bold text-gray-300 tabular-nums">
                {label}
            </span>
        </div>
    );
...
const SortableHeader: React.FC<{
    ...
    return (
        <th scope="col" className={`px-4 py-3 text-[11px] font-extrabold text-gray-400 uppercase tracking-wider cursor-pointer hover:text-white hover:bg-white/5 transition-all normal-case ${className}`}>
            <button onClick={handleClick} className="flex items-center gap-1 group opacity-90">
                <span>{title}</span>
                <div className="flex flex-col -space-y-1 w-2">
                    <span className={`text-[7px] leading-none transition-colors ${isSorted && direction === 'ascending' ? 'text-indigo-400 scale-125' : 'text-gray-600'}`}>▲</span>
                    <span className={`text-[7px] leading-none transition-colors ${isSorted && direction === 'descending' ? 'text-indigo-400 scale-125' : 'text-gray-600'}`}>▼</span>
                </div>
            </button>
        </th>
    );
...
export const ComparisonDashboard: React.FC<ComparisonDashboardProps> = ({ group, data, onBack, dateRange, onDateRangeChange, isHighQuotaFeaturesEnabled, onToggleHighQuotaFeatures }) => {
    ...
    const maxValues = React.useMemo(() => {
        ...
        return {
            subs: Math.max(...data.map(c => parseInt(c.subscriberCount, 10) || 0), 1),
            views: Math.max(...data.map(c => parseInt(c.viewCount, 10) || 0), 1),
            videos: Math.max(...data.map(c => parseInt(c.videoCount, 10) || 0), 1),
        };
    }, [data]);
        ...
        <div className="w-full max-w-7xl mx-auto mt-2 space-y-8">
            ...
            <div className="bg-gray-800/50 rounded-xl p-4 border border-gray-700/50">
                {isHighQuotaFeaturesEnabled ? (
                    ...
                    <div className="flex items-center justify-between">
                        <div>
                            <h3 className="text-lg font-bold text-white">Enable Date Range Comparison</h3>
            ...
            <div className="overflow-x-auto rounded-xl shadow-xl border border-gray-700/50">
                <table className="min-w-full divide-y divide-gray-800/50">
                    ...
                    <tbody className="divide-y divide-gray-800/30">
                        {sortedData.map((channel) => (
                            <tr key={channel.id} className="group hover:bg-white/[0.03] transition-colors">
                                <td className="px-4 py-2 align-middle whitespace-nowrap">
                                    <div className="flex items-center">
                                        <div className="flex-shrink-0 h-10 w-10">
                                            <img className="h-10 w-10 rounded-full border-2 border-gray-700" src={channel.thumbnailUrl} alt="" />
                                        </div>
                                        <div className="ml-4">
                                            <div className="text-[13px] font-bold text-gray-200 group-hover:text-indigo-400 leading-snug">{channel.title}</div>
                                            <div className="text-[9px] text-indigo-400 opacity-60 font-mono">{channel.customUrl.startsWith('@') ? channel.customUrl : `@${channel.customUrl}`}</div>
                                        </div>
                                    </div>
                                </td>
                                <td className="px-4 py-2 align-middle whitespace-nowrap">
                                ...
                                <td className="px-4 py-2 align-middle">
                                    <StatBar value={parseInt(channel.subscriberCount, 10)} max={maxValues.subs} label={formatNumber(channel.subscriberCount)} gradientClass="bg-gradient-to-r from-indigo-500 to-violet-400" />
                                </td>
                                <td className="px-4 py-2 align-middle">
                                    <StatBar value={parseInt(channel.viewCount, 10)} max={maxValues.views} label={formatNumber(channel.viewCount)} gradientClass="bg-gradient-to-r from-emerald-500 to-teal-400" />
                                </td>
                                <td className="px-4 py-2 align-middle">
                                    <StatBar value={parseInt(channel.videoCount, 10)} max={maxValues.videos} label={formatNumber(channel.videoCount)} gradientClass="bg-gradient-to-r from-purple-500 to-fuchsia-400" />
                                </td>
                                <td className="px-4 py-2 align-middle">
