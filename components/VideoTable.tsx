...
}> = ({ label, sortKey, currentSort, currentDirection, onSort, className = "", align = 'left', icon }) => {
        ...
        >
            <div className={`flex items-center gap-1.5 ${align === 'right' ? 'justify-end' : align === 'center' ? 'justify-center' : 'justify-start'}`}>
                ...
                <div className="flex flex-col -space-y-1 w-2">
                    <span className={`text-[7px] transition-colors ${isActive && currentDirection === 'asc' ? 'text-indigo-400 scale-125' : 'text-gray-600'}`}>▲</span>
                    <span className={`text-[7px] transition-colors ${isActive && currentDirection === 'desc' ? 'text-indigo-400 scale-125' : 'text-gray-600'}`}>▼</span>
                </div>
            </div>
        </th>
    );
...
const CopyButton: React.FC<{ text: string }> = ({ text }) => {
    ...
    return (
        <button 
            onClick={handleCopy}
            className={`p-1 rounded-lg transition-all duration-300 ${copied ? 'bg-emerald-500/20 text-emerald-400' : 'text-gray-500 hover:text-white hover:bg-white/10 opacity-0 group-hover/title:opacity-100 shadow-sm'}`}
...
export const VideoTable: React.FC<VideoTableProps> = ({ videos, sortOrder, sortDirection, onSortChange }) => {
    ...
    return (
        <div className="w-full overflow-hidden rounded-[1.5rem] shadow-2xl border border-white/5 font-sans">
            <div className="overflow-x-auto custom-scrollbar">
                <table className="w-full divide-y divide-gray-800/50 table-fixed border-collapse">
                    <thead className="bg-gray-950/30">
                        <tr>
                            <th className="px-6 py-3 text-left text-[11px] font-extrabold text-gray-400 w-auto min-w-[280px] uppercase tracking-wider">
                            ...
                            />
                            <th className="px-4 py-3 text-left text-[11px] font-extrabold text-gray-400 w-[100px] uppercase tracking-wider">
                                <div className="flex items-center gap-1.5 opacity-90">
                                ...
                                <tr key={video.id} className="group hover:bg-white/[0.03] transition-all duration-300">
                                            ...
                                            <div className="min-w-0 flex-1 relative group/title">
                                                <div className="flex items-center gap-2">
                                                    <a 
                                                        ...
                                                        target="_blank" 
                                                        rel="noopener noreferrer"
                                                        className="text-[13px] font-bold text-gray-200 group-hover:text-indigo-400 transition-colors truncate block leading-snug"
                                                        title={video.title}
                                                    >
                                                        {video.title}
                                                    </a>
                                                ...
                                                <div className="flex items-center gap-2 mt-1">
                                                    <span className="text-[9px] text-gray-500 font-medium px-1.5 py-0.5 bg-gray-800/40 rounded border border-white/5">SHORTS</span>
                                                    <span className="text-[9px] text-gray-600 font-mono tracking-tighter opacity-70">ID: {video.id}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-4 py-2 align-middle">
                                        <div className="flex flex-col items-start">
                                            <span className="text-[10px] font-medium text-gray-300">{formatRelativeTime(video.publishedAt)}</span>
                                            <span className="text-[9px] font-medium text-gray-500 mt-0.5">{new Date(video.publishedAt).toLocaleDateString()}</span>
                                        </div>
                                    </td>
                                    <td className="px-4 py-2 align-middle">
                                        <StatBar 
                                    ...
                                    <td className="px-4 py-2 align-middle">
                                        <div className="flex flex-col items-center gap-1">
                                            <span className="text-[11px] font-extrabold text-gray-300 tabular-nums">{formatNumber(video.commentCount)}</span>
                                            <span className="text-[8px] font-black text-gray-600 uppercase tracking-tighter">Comments</span>
                                        </div>
                                    </td>
                                </tr>
                            );
